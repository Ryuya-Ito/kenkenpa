...
var blacktime0 = NSTimer()

var blacktimeover: Float = 20.00

var blackscore: Int = 0

class blackScoregameviewController : UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //1秒ごとに減る
        blacktime0 = NSTimer.scheduledTimerWithTimeInterval(0.01, target: self, selector: Selector("count"), userInfo: nil, repeats: true)
    ...
    func MyButton(sender: UIButton){
        ...
        if myImage1.image == myImageA {
            blackscore+=10
            scoreLabel.text = String(blackscore)
    ...
    func count() {
        ...
        blacktimeover -= 0.01
        timelabel.text = "\(blacktimeover)"
        
        let str0 = "".stringByAppendingFormat("%.2f", blacktimeover)//小数点２桁まで表示
        timelabel.text = str0
        
        if timelabel.text == "0.01" {
            blacktime0.invalidate()
            
            let view: UIViewController = blackScoregameoverviewController()
            
            player?.stop()
            
            let ud = NSUserDefaults.standardUserDefaults()
            ud.setInteger(blackscore, forKey: "blackscore")
