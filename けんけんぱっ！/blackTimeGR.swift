...
class blackTimegameoverviewController : UIViewController ,NADViewDelegate, NADInterstitialDelegate {
    ...
    override func viewDidLoad() {
        ...
        let blacktime = ud.floatForKey("blacktime")
        let str  = String(format: "%.2f",blacktime)
        var blackhi_score1 = ud.floatForKey("blackhi_score1")
        let str0 = String(format: "%.2f",blackhi_score1)
        
        
        //スコアがハイスコアを上回ったら、ハイスコアを更新！
        if blackhi_score1 == 0.00 {
            
            ud.setFloat(blacktime, forKey: "blackhi_score1")
            
            blackhi_score1 = blacktime
        }
        
        if blacktime < blackhi_score1 {
        ...
        hiscorelabel.text = "\(blackhi_score1)"
    ...
    internal func MyButton(sender: UIButton){
        ...
        let Button: UIViewController = blackTimecountviewController()
    ...
    func onPostToTwitter(sender : AnyObject) {
        ...
        let ud = NSUserDefaults.standardUserDefaults()
        _ = ud.integerForKey("blackscore1")
        ...
        // 投稿するテキストを指定.
        let theText = String(format: "けんけんぱっ！　タイムは %.2fでした！みんなもやってみよう！　　ダウンロードはこちらhttps://itunes.apple.com/us/app/kenkenpa-tsu!/id1082624420?mt=8　　         #けんけんぱっ！", blacktimeover1)
