...
class mirrorTimeGRviewController : UIViewController ,NADViewDelegate, NADInterstitialDelegate {
    ...
    override func viewDidLoad() {
        ...
        let mirrortime1 = ud.floatForKey("mirrortime1")
        let str  = String(format: "%.2f",mirrortime1)
        var mirrorhi_score1 = ud.floatForKey("mirrorhi_score1")
        let str0 = String(format: "%.2f",mirrorhi_score1)
        
        
        //スコアがハイスコアを上回ったら、ハイスコアを更新！
        if mirrorhi_score1 == 0.00 {
            
            ud.setFloat(mirrortime1, forKey: "mirrorhi_score1")
            
            mirrorhi_score1 = mirrortime1
        }
        
        if mirrortime1 < mirrorhi_score1 {
        ...
        hiscorelabel.text = "\(mirrorhi_score1)";
    ...
    internal func MyButton(sender: UIButton){
        ...
        let Button: UIViewController = mirrorTimecountviewController()
    ...
    func onPostToTwitter(sender : AnyObject) {
        ...
        let ud = NSUserDefaults.standardUserDefaults()
        _ = ud.integerForKey("mirrortime1")
        ...
        // 投稿するテキストを指定.
        let theText = String(format: "けんけんぱっ！　ミラーモードのタイムは %.2fでし！みんなもやってみよう！　　ダウンロードはこちらhttps://itunes.apple.com/us/app/kenkenpa-tsu!/id1082624420?mt=8　         #けんけんぱっ！", mirrortime)
