...
var ankitime0 = NSTimer()

var ankitimeover: Float = 20.00


var ankiscore: Int = 0

class ankiScoregameviewController : UIViewController {
    ...
    override func viewDidLoad() {
        ...
        //1秒ごとに減る
        ankitime0 = NSTimer.scheduledTimerWithTimeInterval(0.01, target: self, selector: Selector("count"), userInfo: nil, repeats: true)
    ...
    func MyButton(sender: UIButton){
        ...
        if myImage1.image == myImageA {
            ankiscore+=10
            scoreLabel.text = String(ankiscore)
    ...
    func count() {
        ...
        ankitimeover -= 0.01
        timelabel.text = "\(ankitimeover)"
        
        let str0 = "".stringByAppendingFormat("%.2f", ankitimeover)//小数点２桁まで表示
        timelabel.text = str0
        ...
        if timelabel.text == "0.01" {
            ankitime0.invalidate()
            
            let view: UIViewController = ankiScoregameoverviewController()
            
            player?.stop()
            
            let ud = NSUserDefaults.standardUserDefaults()
            ud.setInteger(ankiscore, forKey: "ankiscore")
