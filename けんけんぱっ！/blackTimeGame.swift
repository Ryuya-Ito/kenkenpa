...
var blacktimeover1: Float = 0.00

var blackscore1: Int = 20

var blacktimers = NSTimer()

class blackTimegameviewController : UIViewController , ADFMovieRewardDelegate{
    ...
    override func viewDidLoad() {
        ...
        //0.01秒づつ呼び出す
        blacktimers = NSTimer.scheduledTimerWithTimeInterval(0.01, target: self, selector: Selector("count0"), userInfo: nil, repeats: true)
        ...
        //写真追加
        blackimageview = UIImageView(frame: CGRectMake(0,0,self.view.bounds.width/1,self.view.bounds.height/2.2))
        let countimage = UIImage(named: "kurayami.png")
        blackimageview.image = countimage
        blackimageview.layer.position = CGPoint(x: self.view.bounds.width/2, y: self.view.bounds.height/3)
    ...
    func MyButton(sender: UIButton){
        ...
        if myImage1.image == myImageA {
            ...
            blackscore1-=1
            scoreLabel.text = String(blackscore1)
            ...
            let view: UIViewController = blackMissgameoverviewController()
    ...
    func count0() {
        ...
        blacktimeover1 += 0.01
        timelabel.text = "\(blacktimeover1)"
        
        let str = String(format: "%.2f", blacktimeover1)//小数点２桁まで表示
        timelabel.text = str
        ...
        if scoreLabel.text == "0" {
            ...
            ud.setFloat(blacktimeover1, forKey: "blacktime")
            
            let view: UIViewController = blackTimegameoverviewController()
